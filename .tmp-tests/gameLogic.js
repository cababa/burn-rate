import { GAME_DATA, MAX_HAND_SIZE, ENCOUNTER_TEMPLATES } from './constants.ts';
import { getGlobalLogger } from './logger.ts';
// --- Math Helpers ---
export const calculateDamage = (baseDamage, attackerStatus, defenderStatus, strengthMultiplier = 1, relics) => {
...
const getVulnerableMultiplierInternal = (relics) => {
...
export const countCardsMatches = (cards, matchString) => {
...
export const getRandomRewardCards = (count) => {
    const getRarity = () => {
...
export const drawCards = (currentDraw, currentDiscard, count) => {
    let drawn = [];
...
export const processDrawnCards = (drawn, hand, discard, drawPile, stats, message) => {
    let nextHand = [...hand];
...
export const upgradeCard = (card) => {
    ...
    if (card.id === 'cto_brute_force')
        newDesc = newDesc.replace('3 times', '5 times');
    if (card.id === 'cto_tooling')
        newDesc = newDesc.replace('a card', 'ALL cards');
    return { ...card, name: card.name + "+", effects: newEffects, description: newDesc };
};
// --- Relic Helpers ---
export const applyCombatStartRelics = (currentStats, relics, enemies) => {
...
export const getRelicWoundsToAdd = (relics) => {
    const wounds = [];
    relics.forEach(relic => {
...
export const applyOnCardReward = (relics, currentStats) => {
...
export const getSecretWeaponCard = (relics, deck) => {
...
export const applyCombatEndRelics = (currentStats, relics) => {
    let newStats = { ...currentStats };
    let messages = [];
    relics.forEach(relic => {
...
export const getTurnStartBandwidth = (relics, turn = 1) => {
    let bandwidth = 3;
    relics.forEach(relic => {
...
export const applyTurnEndRelics = (currentStats, relics) => {
    let newStats = { ...currentStats };
    let messages = [];
    relics.forEach(relic => {
...
export const applyOnAttackRelics = (relics, stats) => {
...
export const applyOnEnemyDeathRelics = (relics, stats) => {
    let newStats = { ...stats };
    let drawCards = 0;
    let messages = [];
    relics.forEach(relic => {
...
export const applyOnDamagedRelics = (relics, damageAmount, attackerId) => {
    let thornsDamage = 0;
    let messages = [];
    if (damageAmount > 0) {
        relics.forEach(relic => {
...
export const hasRelic = (relics, relicId) => {
    return relics.some(r => r.id === relicId);
};
export const canRestAtSite = (relics) => {
    return !relics.some(r => r.effect.disable_rest === true);
};
export const getCardLimit = (relics) => {
    for (const relic of relics) {
        if (relic.effect.card_limit)
            return relic.effect.card_limit;
    }
    return null;
};
export const getDrawBonus = (relics) => {
    let bonus = 0;
    relics.forEach(relic => {
        if (relic.effect.draw_bonus)
            bonus += relic.effect.draw_bonus;
    });
    return bonus;
};
export const hasSneckoEffect = (relics) => {
    return relics.some(r => r.effect.type === 'snecko');
};
export const hasRetainHand = (relics) => {
 ...
 */
export const getCrunchModeStrength = (relics, hp, maxHp) => {
 ...
 */
export const getVulnerableMultiplier = (relics) => {
    const growthMindset = relics.find(r => r.effect.type === 'vulnerable_bonus');
    if (growthMindset) {
        return 1.75; // 75% more damage
    }
 ...
 */
export const applyOnHpLossRelics = (relics, damageAmount) => {
    let blockNextTurn = 0;
    let drawCards = 0;
    const messages = [];
    if (damageAmount > 0) {
 ...
 */
export const getPressureCookerWeak = (relics) => {
 ...
 */
export const getPhoenixProtocolDamage = (relics) => {
 ...
 */
export const getHealingMultiplier = (relics) => {
 ...
 */
export const applyMarketDominanceRelics = (relics, stats, enemies) => {
    let newStats = { ...stats, statuses: { ...stats.statuses } };
    let newEnemies = enemies.map(e => ({ ...e, statuses: { ...e.statuses } }));
    const messages = [];
    relics.forEach(relic => {
...
export const getRandomRelic = (rarity, excludeIds = []) => {
 ...
 */
export const getTreasureRelic = (excludeIds = []) => {
    const roll = Math.random() * 100;
 ...
 */
export const getEliteRelic = (excludeIds = []) => {
    const roll = Math.random() * 100;
 ...
 */
export const getBossRelicChoices = (excludeIds = []) => {
 ...
 */
export const getBossRelicSkipGold = () => {
 ...
 */
export const getAvailablePotions = (character = 'cto') => {
    return Object.values(GAME_DATA.potions).filter(p => p.character === 'shared' || p.character === character);
};
/**
 * Get potions by rarity
 */
export const getPotionsByRarity = (rarity, character = 'cto') => {
 ...
 */
export const generateRandomPotion = (character = 'cto', excludeFruitJuice = false) => {
    const roll = Math.random() * 100;
    let rarity;
    if (roll < 65)
        rarity = 'common';
 ...
 */
export const checkPotionDrop = (currentChance) => {
    const roll = Math.random() * 100;
 ...
 */
export const canAcquirePotion = (potions) => {
    return potions.some(p => p === null);
};
/**
 * Add potion to first empty slot
 */
export const addPotionToSlot = (potions, potion) => {
 ...
 */
export const removePotionFromSlot = (potions, slotIndex) => {
    const newPotions = [...potions];
 ...
 */
export const getSacredBarkMultiplier = (relics) => {
 ...
 */
export const initializePotionSlots = (slotCount = 3) => {
 ...
 */
export const checkFairyPotion = (potions, maxHp, relics) => {
 ...
 */
export const canUseExitStrategy = (enemies) => {
 ...
 */
export const resolvePotionEffect = (state, potion, slotIndex, targetEnemyId) => {
    let newState = { ...state };
...
const STS_MAP = {
    ...
    // Fixed floors
    FLOOR_1_TYPE: 'problem', // All floor 1 = normal fights
    FLOOR_9_TYPE: 'treasure', // All floor 9 = chest
    FLOOR_15_TYPE: 'retrospective', // All floor 15 = rest
...
const buildPathSpines = () => {
    const paths = [];
    const usedStartPositions = new Set();
    for (let pathIdx = 0; pathIdx < STS_MAP.NUM_PATHS; pathIdx++) {
        ...
        for (let floor = 2; floor <= STS_MAP.FLOORS; floor++) {
            ...
            const possibleMoves = [];
            if (currentCol > 0)
                possibleMoves.push(currentCol - 1);
            possibleMoves.push(currentCol);
            if (currentCol < STS_MAP.LANES - 1)
                possibleMoves.push(currentCol + 1);
...
const wouldCrossPath = (existingPaths, floor, fromCol, toCol) => {
    for (const path of existingPaths) {
        if (path.length <= floor)
            continue;
        const otherFromCol = path[floor - 1];
        const otherToCol = path[floor];
        // Crossing occurs if paths swap relative positions
        if (fromCol < otherFromCol && toCol > otherToCol)
            return true;
        if (fromCol > otherFromCol && toCol < otherToCol)
            return true;
    }
...
const buildNodesFromPaths = (paths) => {
    const nodes = new Map();
...
const assignRoomTypes = (nodes) => {
    ...
    for (let i = 0; i < targetCounts.vendor; i++)
        bucket.push('vendor');
    for (let i = 0; i < targetCounts.retrospective; i++)
        bucket.push('retrospective');
    for (let i = 0; i < targetCounts.opportunity; i++)
        bucket.push('opportunity');
    for (let i = 0; i < targetCounts.elite; i++)
        bucket.push('elite');
    ...
    untypedKeys.sort((a, b) => {
        ...
        if (nodeA.floor !== nodeB.floor)
            return nodeA.floor - nodeB.floor;
    ...
    if (type === 'elite' || type === 'retrospective' || type === 'vendor' || type === 'opportunity') {
        ...
        for (const parentKey of node.parentConnections) {
            ...
            for (const siblingKey of parentNode.connections) {
                if (siblingKey === `${node.floor}_${node.column}`)
                    continue;
...
export const getEncounterForFloor = (floor) => {
    ...
    selectedTemplate.enemies.forEach(enemyConfig => {
        ...
        const baseEnemy = GAME_DATA.enemies[enemyConfig.enemyId];
        if (baseEnemy) {
            for (let i = 0; i < count; i++) {
...
// Get Elite Encounter
export const getEliteEncounter = () => {
...
// --- Game Logic ---
// --- AI Logic ---
const getNextIntent = (enemy, turn, playerHp) => {
    const roll = Math.random() * 100;
    // --- COMMON ---
    if (enemy.id.startsWith('fanboy')) {
        if (turn === 1)
            return { type: 'buff', value: 3, icon: 'buff', description: "Ritual (Hype)" };
    ...
    if (enemy.id.startsWith('spaghetti_code')) {
        if (roll < 25)
            return { type: 'buff', value: 3, icon: 'buff', description: "Bellow (Block+Str)" };
        if (roll < 55)
            return { type: 'attack', value: 7, icon: 'attack', description: "Thrash (7 Dmg + 5 Blk)" };
        return { type: 'attack', value: 11, icon: 'attack', description: "Chomp (11 Dmg)" };
    }
    if (enemy.id.startsWith('critical_bug')) {
        if (roll < 25)
            return { type: 'buff', value: 3, icon: 'buff', description: "Grow (Severity)" };
        const dmg = 6 + enemy.statuses.strength;
        return { type: 'attack', value: dmg, icon: 'attack', description: "Critique" };
    }
    if (enemy.id.startsWith('minor_bug')) {
        if (roll < 25)
            return { type: 'debuff', value: 2, icon: 'debuff', description: "Spit Web (Weak)" };
    ...
    if (enemy.id.startsWith('quick_hack') || enemy.id.startsWith('hotfix')) {
        if (roll < 50)
            return { type: 'attack', value: 3, icon: 'attack', description: "Quick Commit" };
    ...
    if (enemy.id.startsWith('tech_debt') || enemy.id.startsWith('bad_merge')) {
        if (roll < 30)
            return { type: 'debuff', value: 1, icon: 'debuff', description: "Add Complexity" };
    ...
    if (enemy.id.startsWith('legacy_module') || enemy.id.startsWith('merge_conflict')) {
        if (roll < 30)
            return { type: 'debuff', value: 2, icon: 'debuff', description: "Add Complexity" };
    ...
    if (enemy.id.startsWith('feature_pusher')) {
        if (turn === 1)
            return { type: 'debuff', value: 1, icon: 'debuff', description: "Entangle" };
        if (roll < 45) {
    ...
    if (enemy.id.startsWith('headhunter')) {
        if (turn >= 3 && roll < 15)
            return { type: 'buff', value: 0, icon: 'buff', description: "Escape (Flee)" };
        if (roll < 50) {
    ...
    if (enemy.id.startsWith('memory_leak')) {
        if (roll < 40)
            return { type: 'buff', value: 3, icon: 'buff', description: "Grow (Strength)" };
        const dmg = 6 + enemy.statuses.strength;
    ...
    if (enemy.id.startsWith('scope_creep')) {
        if (turn === 1)
            return { type: 'buff', value: 2, icon: 'buff', description: "Bellow (Enrage)" };
        if (roll < 33) {
            const dmg = 6 + enemy.statuses.strength;
    ...
    if (enemy.id.startsWith('legacy_monolith') || enemy.id.startsWith('legacy_hack') || enemy.id.startsWith('legacy_patch')) {
        if (turn % 2 === 0)
            return { type: 'attack', value: 9, icon: 'attack', description: "Beam" };
    ...
    if (enemy.id.startsWith('boss_the_pivot')) {
        ...
        if (isDefensive) {
            ...
            const cycle = turn % 4;
            if (cycle === 1)
                return { type: 'buff', value: 9, icon: 'buff', description: "Charging Up (Block)" };
            if (cycle === 2)
                return { type: 'attack', value: 32, icon: 'attack', description: "Fierce Bash" };
            if (cycle === 3)
                return { type: 'debuff', value: 2, icon: 'debuff', description: "Vent Steam (Vuln+Weak)" };
            return { type: 'attack', value: 20, icon: 'attack', description: "Whirlwind (x4)" };
        }
    }
    if (enemy.id.startsWith('boss_burn_rate')) {
        if (turn === 1)
            return { type: 'attack', value: 0, icon: 'attack', description: "Divider (x6)" };
        const cycle = (turn - 1) % 5;
        if (cycle === 0)
            return { type: 'attack', value: 6, icon: 'attack', description: "Sear" };
        if (cycle === 1)
            return { type: 'debuff', value: 1, icon: 'debuff', description: "Burn" };
        if (cycle === 2)
            return { type: 'attack', value: 6, icon: 'attack', description: "Sear" };
        if (cycle === 3)
            return { type: 'debuff', value: 2, icon: 'debuff', description: "Burn" };
        return { type: 'attack', value: 2, icon: 'attack', description: "Inferno (x6)" };
    }
    if (enemy.id.startsWith('boss_the_monolith')) {
        const cycle = turn % 3;
        if (cycle === 1)
            return { type: 'debuff', value: 3, icon: 'debuff', description: "Goop Spray (Slimed)" };
        if (cycle === 2)
            return { type: 'buff', value: 0, icon: 'buff', description: "Prepare" };
        return { type: 'attack', value: 35, icon: 'attack', description: "Slam" };
    }
    return { type: 'attack', value: 5, icon: 'attack', description: "Attack" };
};
export const resolveEndTurn = (prev) => {
    ...
    if (nextPlayerStatuses.vulnerable > 0) {
        ...
        if (nextPlayerStatuses.vulnerable === 0)
            getGlobalLogger().log('STATUS_EXPIRED', 'Exposed expired');
    }
    if (nextPlayerStatuses.weak > 0) {
        nextPlayerStatuses.weak--;
        if (nextPlayerStatuses.weak === 0)
            getGlobalLogger().log('STATUS_EXPIRED', 'Drained expired');
    }
    if (nextPlayerStatuses.frail > 0) {
        nextPlayerStatuses.frail--;
        if (nextPlayerStatuses.frail === 0)
            getGlobalLogger().log('STATUS_EXPIRED', 'Frail expired');
    }
    // Clear No Draw
...
export const resolveEnemyTurn = (prev) => {
    ...
    newEnemies.forEach(enemy => {
        ...
        if (enemy.statuses.vulnerable > 0) {
            ...
            if (enemy.statuses.vulnerable === 0)
                getGlobalLogger().log('STATUS_EXPIRED', `${enemy.name} Exposed expired.`);
        }
        if (enemy.statuses.weak > 0) {
            enemy.statuses.weak--;
            if (enemy.statuses.weak === 0)
                getGlobalLogger().log('STATUS_EXPIRED', `${enemy.name} Drained expired.`);
        }
    ...
    let pendingPotionReward = undefined;
...
export const resolveCardEffect = (prev, card, target, targetEnemyId) => {
        ...
        for (let i = 0; i < loops; i++) {
            if (effect.type === 'damage') {
                ...
                targets.forEach(t => {
                    ...
                    if (t.statuses.vulnerable > 0)
                        newMessage += " (Exposed!)";
                    if (newPlayerStatuses.weak > 0)
                        newMessage += " (Drained...)";
                    if (t.mitigation > 0) {
                ...
                if (effect.target === 'self') {
                    if (statusType === 'strength')
                        newPlayerStatuses.strength += amount;
                    if (statusType === 'metallicize')
                        newPlayerStatuses.metallicize += amount;
                    if (statusType === 'evolve')
                        newPlayerStatuses.evolve += amount;
                    if (statusType === 'feelNoPain')
                        newPlayerStatuses.feelNoPain += amount;
                    if (statusType === 'noDraw')
                        newPlayerStatuses.noDraw += amount;
                    if (statusType === 'thorns')
                        newPlayerStatuses.thorns += amount;
                    if (statusType === 'antifragile')
                        newPlayerStatuses.antifragile += amount;
                    if (statusType === 'artifact')
                        newPlayerStatuses.artifact += amount;
                    if (statusType === 'growth')
                        newPlayerStatuses.growth += amount;
                    if (statusType === 'corruption')
                        newPlayerStatuses.corruption += amount;
                }
                    ...
                    if (statusType === 'weak')
                        targetEnemy.statuses.weak += amount;
                    if (statusType === 'strength')
                        targetEnemy.statuses.strength += amount; // Disarm (negative strength)
                }
            }
