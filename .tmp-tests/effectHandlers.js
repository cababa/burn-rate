...
// UTILITY FUNCTIONS
// ============================================
export function calculateDamageValue(baseDamage, attackerStatuses, defenderStatuses, strengthMultiplier = 1, relics = []) {
 ...
 */
export const handleDamage = (ctx, result) => {
    ...
    targets.forEach(target => {
        const damage = calculateDamageValue(effect.value, playerStatuses, target.statuses, effect.strengthMultiplier || 1, relics);
 ...
 */
export const handleBlock = (ctx, result) => {
 ...
 */
export const handleDraw = (ctx, result) => {
    const { effect, playerStatuses } = ctx;
 ...
 */
export const handleApplyStatus = (ctx, result) => {
    const { effect } = ctx;
 ...
 */
export const handleAddCopy = (ctx, result) => {
    const { card } = ctx;
 ...
 */
export const handleGainBandwidth = (ctx, result) => {
    const { effect } = ctx;
 ...
 */
export function executeEffectHandler(effectType, ctx, currentResult) {
 ...
 */
export function createInitialResult(state, card) {
    return {
