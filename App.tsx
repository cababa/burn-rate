...
const getRandomRewardCards = (count: number): CardData[] => {
    // Include new cards in pool
    const pool = Object.values(GAME_DATA.cards).filter(c => c.type !== 'status' && !c.id.startsWith('cto_commit') && !c.id.startsWith('cto_rollback'));
...
const App: React.FC = () => {
  ...
  const playCard = (card: CardData, target: 'enemy' | 'self') => {
    ...
    setGameState(prev => {
        ...
        while(cardsToCheck.length > 0 && safetyCap < 20) {
            ...
            // Legacy Code Drain
            if (checking.effects.some(e => e.type === 'lose_bandwidth' && checking.unplayable)) {
                 newBandwidth -= 1;
  ...
  const processEnemyTurn = () => {
      setGameState(prev => {
          ...
          if (newPlayerStatuses.antifragile > 0 && newStatus === 'PLAYING') {
               newPlayerStatuses.strength += newPlayerStatuses.antifragile;
               newMessage += ` (Antifragile: +${newPlayerStatuses.antifragile} STR)`;
          }
